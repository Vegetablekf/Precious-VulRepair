...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, train_data=None, val_data=None, file_type="train"):
        ...
        for i in tqdm(range(len(sources))):
            # convert_examples_to_features返回一个对象，有三个参数，分别是source/target/target对应的词表的序号
    ...
    # self.examples[i].input_ids.ne(0)   某项为0，则返回1，为1，则返回0，这样，有单词的token的mask为1，补全的token的mask为0
...
def convert_examples_to_features(source, label, tokenizer, args):
    ...
    # 具体来说，tokenizer.encode() 接受一个字符串作为输入，然后使用一个预训练的文本分词器（tokenizer）将其分成一个个单独的词或子词，
    # 并为每个词或子词分配一个唯一的数字编码。最后，函数返回一个数字序列，其中每个数字代表原始文本中的一个词或子词。
    # return_tensors有三个可选的值，分别是None、'pt'和'tf'，分别对应着不返回任何结果、返回 PyTorch 张量和返回 TensorFlow 张量
    # padding 将输出数字序列填充到max_length
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    # if not any(nd in n for nd in no_decay) ：对于任何n,只要'bias' 或者'LayerNorm.weight' 在n中，就返回false
    # p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay) ：
    # 选取model.named_parameters()中不包含no_decay里单词的参数
    # p for n, p in model.named_parameters() if any(nd in n for nd in no_decay) ：
    # 选取model.named_parameters()中包含no_decay里单词的参数
    # decay ： 衰退   args.weight_decay = 0.0
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            # 这里我的理解是input_ids这些从(1X512)向量变成了(512X1)向量
...
def main():
    ...
    # Set seed
    set_seed(args)
    # 创建一个RobertaTokenizer实例，用到的模型是Salesforce/codet5-base
    ...
    if args.do_train:
        # 下面为4月25日所做的更改
        # model.load_state_dict(torch.load(r'./saved_models/checkpoint-best-loss/model_TgtOnlyCWE.bin', map_location=args.device))
        train_data_whole = datasets.load_dataset("./dataset",split="train", encoding = 'utf-8')
        # 下面这句形成两列，source target
        ...
        # TextDataset()  输出一个对象，包含input  lable  decoder_input的ids
